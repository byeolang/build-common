...
`build-common` is one of those six repositories: the shared CMake rules
that every other byeol-org repository pulls in. When you need deeper
information on a specific topic, follow the link in the routing table
rather than looking for a local Markdown file — the detailed documents
live on byeol.io, generated from `byeol`'s `doc/`.
